...
export async function getWeekSummary() {
  ...
  const goalsCreatedUpToWeek = db.$with('goals_created_up_to_week').as(db
    .select({
      id: goals.id,
      title: goals.title,
  ...
  const goalsCompletedInWeek = db.$with('goals_completed_in_week').as(db
    .select({
  ...
  const goalsCompletedByWeekDay = db.$with('goals_completed_by_week_day').as(db
    .select({
